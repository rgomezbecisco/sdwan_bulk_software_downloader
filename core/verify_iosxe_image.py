from utils.additional_functions import (
    ...
    confirm_iosxe_image_verification,
...
def run_verify_iosxe_image(target_manager, filename=None, custom_timeout=600, verbose=False, sshclient=None, reuse_session=False):
    ...
    if not filename:
        tprint("IOS XE image verification FAILED! No image mapped for this device model")
    ...
    verify_ok = result.get("success") and confirm_iosxe_image_verification(result.get("output"))
    if verify_ok:
        tprint(f"IOS XE image verification successful!")

    else:
        tprint(f"IOS XE image verification FAILED!")

    # Propagate semantic validation so callers can make correct status decisions.
    result["success"] = bool(verify_ok)
...
if __name__ == "__main__":
    ...
    result = run_verify_iosxe_image(target_manager, verbose=verbose)
